...
/**
 * Handler für das Aktualisieren von Zonen
 */
export class ZoneUpdateHandler {
  ...
  async handle(message: string): Promise<void> {
    try {
      ...
      // Aktualisiere die Zone in Discord
      await this.channelService.updateVoiceChannel(zone);
    } catch (error) {
      console.error('Error handling zoneUpdated event:', error);
    }
  }
